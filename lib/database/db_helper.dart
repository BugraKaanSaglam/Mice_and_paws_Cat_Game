...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    await db
        .execute('CREATE TABLE OPCGameTable(Ver INTEGER not null PRIMARY KEY, LanguageCode INTEGER not null, MusicVolume DOUBLE not null, MiceVolume DOUBLE not null)');
  }
  ...
  Future<OPCDataBase?> getList(int ver) async {
    ...
    List<Map<String, dynamic>> maps =
        await dbClient!.query('OPCGameTable', columns: ['Ver', 'LanguageCode', 'MusicVolume', 'MiceVolume'], where: 'Ver = ?', whereArgs: [ver]);
    if (maps.isNotEmpty) {
